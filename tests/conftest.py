...
def mock_notion_client():
    ...
    with patch('feed_to_somewhere.notion_client.Client') as mock_client_class:
...
def mock_feedparser():
    ...
    with patch('feed_to_somewhere.feed_processor.feedparser') as mock_feedparser:
...
def mock_requests():
    ...
    with patch('feed_to_somewhere.feed_processor.requests') as mock_requests:
        yield mock_requests
...
def mock_bs4():
    ...
    with patch('feed_to_somewhere.feed_processor.BeautifulSoup') as mock_bs:
        yield mock_bs
...
def mock_thread_pool():
    ...
    with patch('feed_to_somewhere.feed_processor.concurrent.futures.ThreadPoolExecutor') as mock_executor_class:
...
def mock_logger():
    ...
    with patch('feed_to_somewhere.logger.logger') as mock_logger:
        yield mock_logger
